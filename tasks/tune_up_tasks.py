...
def build_tune_up_tasks(
    ...
    verify_description = (
        ...
        "   - Timeline realism\n"
        "   - Transit/backtracking (IMPORTANT: each day's activities should be grouped by geographic area)\n"
        "   - General and solo-female-aware safety\n"
        "   - Rough budget fit\n"
        "   - Overall coherence\n"
        "2) Suggest specific, concrete improvements (not generic advice).\n"
        "3) If activities are spread across multiple distant areas in one day, suggest regrouping by location.\n\n"
        "OUTPUT FORMAT:\n"
        "### TUNED ITINERARY (DRAFT)\n"
        "[Separate table per day: ## Day X heading, then table with Time | Activity | Location | Transport | Notes]\n"
        "[IMPORTANT: Cluster activities by geographic area per day]\n\n"
        "### ISSUES FOUND\n"
        "- [Issue 1]\n"
        "- [Issue 2]\n\n"
        "### CHECKLIST\n"
        "- Timeline: [OK / Needs changes + why]\n"
        "- Geographic clustering: [OK / Needs changes + why] (are activities per day grouped by area?)\n"
    ...
    polish_description = (
        ...
        "IMPORTANT: Ensure activities within each day are grouped by geographic area (no backtracking)\n\n"
        "TASK:\n"
        "- Keep the same structure and decisions unless something is clearly unsafe or unrealistic.\n"
        "- Verify each day's activities are geographically clustered (grouped by area/neighborhood).\n"
        ...
        "[Separate table per day: ## Day X heading, then table with Time | Activity | Location | Transport | Notes]\n"
        "[All activities per day grouped by geographic area]\n\n"
